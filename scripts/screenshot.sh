...
SAVE_DIR=$(cat ~/archman-dots/scripts/settings/screenshot-folder)
SAVE_FILENAME=$(cat ~/archman-dots/scripts/settings/screenshot-filename)
eval screenshot_folder="$SAVE_DIR"
eval NAME="$SAVE_FILENAME"

# Notifications
source "$HOME/archman-dots/scripts/notification-handler"
APP_NAME="Screen Capture"
NOTIFICATION_ICON="camera-photo-symbolic"

# Screenshot Editor
export GRIMBLAST_EDITOR="$(cat $HOME/archman-dots/scripts/settings/screenshot-editor)"
